...
async function main() {
  try {
    ...
    // 7. Git repository'yi hazırla
    console.log('\n🔄 Git repository hazırlanıyor...');
    await prepareGitRepo();

    console.log('\n✅ Yedekleme başarıyla tamamlandı!');
    console.log(`📊 Yedek boyutu: ${getDirectorySize(BACKUP_DIR)}`);
    console.log('\n📋 Manuel push için:');
    console.log(`cd "${BACKUP_DIR}/repo"`);
    console.log('git add .');
    console.log('git commit -m "Tam yedekleme"');
...
async function prepareGitRepo() {
  ...
  try {
    ...
    console.log('  ✅ Git repository hazırlandı!');
    
  } catch (error) {
    console.error('  ❌ Git repository hazırlama hatası:', error.message);
    throw error;
  }
}
